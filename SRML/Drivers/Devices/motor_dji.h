...
{
	CAN_COB Id200 = {0x200,8,{0}};
	CAN_COB Id1ff = {0x1ff,8,{0}};
	CAN_COB Id2ff = {0x2ff,8,{0}};
};
/* Exported variables --------------------------------------------------------*/
...
Motor_CAN_COB &MotorMsgPack(Motor_CAN_COB &motor_msg, MotorType &motor)
{
    ...
    if (motor.ID <= 4 && motor.ID > 0){
    	...
    	if (motor.SEND_ID_HIGH() == motor_msg.Id1ff.ID){
			...
			motor_msg.Id2ff.Data[motor.ID * 2 - 2] = (constrain_Out >> 8) & 0xff;
			motor_msg.Id2ff.Data[motor.ID * 2 - 1] = constrain_Out & 0xff;
		}
    }
    return motor_msg;
}
